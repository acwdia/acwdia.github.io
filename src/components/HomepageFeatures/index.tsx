...
export default function HomepageFeatures(): ReactNode {
  ...
  const allItems = ((siteConfig.themeConfig as any).navbar?.items ?? []) as CourseNavItem[];
  const courseItems = allItems.filter(item => item.type === 'docSidebar');
      ...
      <div className="container">
        <div className={styles.courseGrid}>
          {courseItems.map((course, idx) => {
            const colors = COVER_COLORS[idx % COVER_COLORS.length];
            return (
              <Link
                key={course.label}
